def filter_by_currency(transactions: list[dict], currency: str):
    """Возвращает итератор, который поочередно выдает транзакции, где валюта операции соответствует заданной."""
    found = False
    for transaction in transactions:
        if transaction["operationAmount"]["currency"]["name"] == currency:
            found = True
            yield transaction

    if not found:
        raise AttributeError("Операции с заданной валютой не существуют!")


def transaction_descriptions(transactions: list[dict]):
    """Возвращает описание каждой операции по очереди"""
    if not transactions:
        yield "Описание отсутсвует"
    else:
        for transaction in transactions:
            yield transaction.get("description", "Описание отсутсвует")


def card_number_generator(start_value: int, end_value: int):
    """Генерирует номера карт в задонном диапазоне"""
    for num in range(start_value, end_value + 1):
        number = "0" * (16 - len(str(num))) + str(num)
        card_number = f"{number[:4]} {number[4:8]} {number[8:12]} {number[12:]}"
        yield card_number


transactions = [
    {
        "id": 939719570,
        "state": "EXECUTED",
        "date": "2018-06-30T02:08:58.425572",
        "operationAmount": {"amount": "9824.07", "currency": {"name": "USD", "code": "USD"}},
        "description": "Перевод организации",
        "from": "Счет 75106830613657916952",
        "to": "Счет 11776614605963066702",
    },
    {
        "id": 142264268,
        "state": "EXECUTED",
        "date": "2019-04-04T23:20:05.206878",
        "operationAmount": {"amount": "79114.93", "currency": {"name": "USD", "code": "USD"}},
        "description": "Перевод со счета на счет",
        "from": "Счет 19708645243227258542",
        "to": "Счет 75651667383060284188",
    },
    {
        "id": 873106923,
        "state": "EXECUTED",
        "date": "2019-03-23T01:09:46.296404",
        "operationAmount": {"amount": "43318.34", "currency": {"name": "руб.", "code": "RUB"}},
        "description": "Перевод со счета на счет",
        "from": "Счет 44812258784861134719",
        "to": "Счет 74489636417521191160",
    },
    {
        "id": 895315941,
        "state": "EXECUTED",
        "date": "2018-08-19T04:27:37.904916",
        "operationAmount": {"amount": "56883.54", "currency": {"name": "USD", "code": "USD"}},
        "description": "Перевод с карты на карту",
        "from": "Visa Classic 6831982476737658",
        "to": "Visa Platinum 8990922113665229",
    },
    {
        "id": 594226727,
        "state": "CANCELED",
        "date": "2018-09-12T21:27:25.241689",
        "operationAmount": {"amount": "67314.70", "currency": {"name": "руб.", "code": "RUB"}},
        "description": "Перевод организации",
        "from": "Visa Platinum 1246377376343588",
        "to": "Счет 14211924144426031657",
    },
]
# # def get_transactions(currency):
# #     return filter_by_currency(transactions, currency)
# #
# # usd_transactions = get_transactions("EUR")
# # print(next(usd_transactions))  # Получаем первый элемент
# #
# # # Вызываем функцию снова, чтобы получить новый генератор
# # usd_transactions = get_transactions("EUR")
# # print(next(usd_transactions))
# def filter_by_currency(transactions: list[dict], currency: str):
#     """
#     Генератор, фильтрующий транзакции по валюте.
#     """
#     for transaction in transactions:
#         if transaction["operationAmount"]["currency"]["name"] == currency:
#             yield transaction  # Просто отдаём подходящие транзакции
#
# # Данные
# transactions = [
#     {
#         "id": 1,
#         "operationAmount": {"amount": "100", "currency": {"name": "USD"}}
#     },
#     {
#         "id": 2,
#         "operationAmount": {"amount": "200", "currency": {"name": "EUR"}}
#     },
# ]
#
# # Использование генератора
# currency = "GBP"
# # filtered_transactions = filter_by_currency(transactions, currency)
# #
# # try:
# #     transaction = next(filtered_transactions)  # Пробуем взять первый элемент
# #     print(transaction)
# # except StopIteration:
# #     print("Операции с заданной валютой не существуют!")

# print(next(filter_by_currency([{
#           "id": 939719570,
#           "state": "EXECUTED",
#           "date": "2018-06-30T02:08:58.425572",
#           "operationAmount": {
#               "amount": "9824.07",
#               "currency": {
#                   "name": "USD",
#                   "code": "USD"
#               }
#           },
#           "description": "Перевод организации",
#           "from": "Счет 75106830613657916952",
#           "to": "Счет 11776614605963066702"
#       },
#       {
#               "id": 142264268,
#               "state": "EXECUTED",
#               "date": "2019-04-04T23:20:05.206878",
#               "operationAmount": {
#                   "amount": "79114.93",
#                   "currency": {
#                       "name": "USD",
#                       "code": "USD"
#                   }
#               },
#               "description": "Перевод со счета на счет",
#               "from": "Счет 19708645243227258542",
#               "to": "Счет 75651667383060284188"
#        }], "EUR")))


# try:
#     generator = filter_by_currency([], "GBP")
#     print(next(generator))
# except AttributeError  as e:
#     print(f"Ошибка: {e}")
