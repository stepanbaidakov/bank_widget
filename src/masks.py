...
def get_mask_card_number(card_number: int) -> str:
    ...
    if card_number != "":
        mask_card_number_logger.info("Номер карты маскируется")
        card_number_str = str(card_number)
        masked_number = f"{card_number_str[:4]} {card_number_str[4:6]}** **** {card_number_str[-4:]}"
        return masked_number
    else:
        mask_card_number_logger.warning("Номер карты не был указан")
        return ""
...
def get_mask_account(account_number: int) -> str:
    ...
    if account_number != "":
        mask_account_logger.info("Номер счета маскируется")
        account_number_str = str(account_number)
        masked_number = f"**{account_number_str[-4:]}"
        return masked_number
    else:
        mask_account_logger.error("Номер счета не был указан")
        return ""
